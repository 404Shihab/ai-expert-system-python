...
def dfs(startingNode, destinationNode, G):
    ...
    while len(frontier) > 0:
        print(frontier)
        unode = frontier.pop()     # selecting the node to expand (STACK)
        u = unode['City']
        expand_sequence.append(u)
        print(u + " expanding..")
        # expand u

        # reverse order so DFS follows the same traversal order as expected
        for v in reversed(list(G[u].keys())):
...
dfs('Arad', 'Bucharest', romanian_map) # change the source and destination